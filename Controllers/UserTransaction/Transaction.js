...
class UserTrans extends Notifications {
  ...
  async createTransaction(body) {
    ...
    try {
      const slab = await collections.slabCollection().findOne({ type: "knoone" });

      if (!slab) {
        return tryAgain;
      }

      trans.charges = parseFloat(slab.tax);
      trans.amount = parseFloat(body.storage) >= 1
        ? parseInt(body.storage * slab.basicAmt)
        : parseInt(slab.range);
      ...
      if (user && user.userId) {
        ...
        if (result && result.insertedId) {
          let option = options(user.email, "Kno-one Transaction Code Generated", transaction(user.userId, trans.amount, result.insertedId, user.fullName));
